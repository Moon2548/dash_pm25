def eng_model_predict_2_5(days):
    ...
    train_data, test_data = train_test_split(
        data_filtered, test_size=test_size, shuffle=False
    )
    ...
    result_plot_temp = pd.concat(
        [result["prediction_label"].shift(-1), real_data["temperature"]], axis=1
    )
    ...
    result_plot_humi = pd.concat(
        [result_humi["prediction_label"].shift(-1), real_data["humidity"]], axis=1
    )
...
# output return เป็น dataframe ที่มี prediction_label, actual_temp ซึ่งจะเลือกให้แสดงผล prediction_label อย่างเดียว
def eng_model_predict_temp(days):
    num_days = days  # วันที่ต้องการทำนาย
...
def eng_model_predict_humidity(days):
    num_days = days  # วันที่ต้องการทำนาย
